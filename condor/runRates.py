...
# --- Sample Dictionary ---
sample_dic = samples_pfmc
executable = 'condorRatesMC.sh'
...
outDir='/store/user/lpchtop/BBto2b4tau_May2026_PromptFake/'
condorDir='/uscms/home/jmanagan/nobackup/TTBBto2TB4Tau/condor_May2026_PromptFake/' # recommend this be outside git area!
...
if runanalyzer:    
    ...
    for k,v in sample_dic.items(): # This is fine.  It is a dictionary that exists in samples.py that gets imported at runtime
        ...
        for i in range(filesPerJob,num+filesPerJob,filesPerJob):
            ...
            dict={'RUNDIR':runDir, 'CONDORDIR':condorDir+'/'+prefix, 'CMSSWBASE':relbase, 'OUTPUTDIR':outDir, 'TARBALL':tarfile, 'TESTNUM1':oldNum, 'TESTNUM2':newNum-1, 'PREFIX':prefix, 'FILENAME':fileName, 'YEAR':year, 'EXEC':executable}
